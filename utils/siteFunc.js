...
var siteFunc = {
  ...
  setConfirmPassWordEmailTemp: function(res, sysConfigs, name, token) {
    ...
    var html = '<p>' + res.__("label_sendActiveEmail_text1") + '：' + name +
      '</p><br/>' + '<p>' + res.__("label_sendActiveEmail_text2") +
      '</p><br/>' + '<p><strong>' + siteTitle + '</strong> ' + res.__(
        "label_sendActiveEmail_text2_1") + '</p><br/><br/>' + '<p>' + res.__(
        "label_sendActiveEmail_text3") + '</p><br/>' + '<a href="' +
      sysConfigs.siteDomain + '/users/reset_pass?key=' + token + '">' + res
      .__("label_sendActiveEmail_text4") + '</a><br/>' + '<a href="' +
      sysConfigs.siteDomain + '/users/reset_pass?key=' + token + '">' +
      sysConfigs.siteDomain + '/users/reset_pass?key=' + token +
      '</a><br/>' + '<p> <strong>' + siteTitle + ' </strong> </p>';
    return html;
  },
  ...
  setNoticeToAdminEmailTemp: function(res, sysConfigs, obj) {
    ...
    var html = '';
    html += '主人您好，<strong>' + obj.author.userName + '</strong> 于 ' +
      msgDate + ' 在 <strong>' + siteTitle + '</strong> 的文章 <a href="' +
      sysConfigs.siteDomain + '/details/' + obj.content._id + '.html">' +
      obj.content.title + '</a> 中留言了';
    return html;
  },
  ...
  setNoticeToAdminEmailByContactUsTemp: function(res, sysConfigs, obj) {
    ...
    var msgDate = moment(obj.date).format('YYYY-MM-DD HH:mm:ss');
    var html = '';
    html += res.__("lc_sendEmail_user_notice_title") + '<br/><br/>' + res.__(
        "lc_sendEmail_user_success_notice") + '<br/><br/>' + res.__(
        "lc_sendEmail_user_notice_Info") + '<br/><br/>' + '<strong>' + res.__(
        "label_user_email") + ': </strong>' + obj.email + '<br/><br/>' +
      '<strong>' + res.__("label_user_phoneNum") + ': </strong>' + obj.phoneNum +
      '<br/><br/>' + '<strong>' + res.__("lc_sendEmail_user_content") +
      ': </strong><br/><br/>' + obj.comments + '<br/><br/>'
    return html;
  },

  setNoticeToUserByContactUsTemp: function(res, sysConfigs, obj) {
    var html = '';
    html += res.__("label_sendActiveEmail_text1") + '，<strong>' + obj.name +
      '</strong>' + res.__("lc_sendEmail_user_success_notice") +
      '<br/><br/>';
    return html;
  },
  ...
  setNoticeToUserEmailTemp: function(res, sysConfigs, obj) {
    ...
    html += '主人您好，<strong>' + targetEmail + '</strong> 于 ' + msgDate +
      ' 在 <strong>' + siteTitle + '</strong> 的文章 <a href="' + sysConfigs.siteDomain +
      '/details/' + obj.content._id + '.html">' + obj.content.title +
      '</a> 中回复了您';
    return html;
  },
  ...
  setBugToAdminEmailTemp: function(res, sysConfigs, obj) {
    ...
    var html = '';
    html += '主人您好，测试管理员（' + obj.email + ')于 ' + msgDate + ' 在 <strong>' +
      siteTitle + '</strong> 的后台模块 <strong>' + obj.contentFrom +
      '</strong> 中说：<br>' + obj.content;
    return html;
  },
  ...
  setNoticeToUserRegSuccess: function(res, sysConfigs, obj) {
    let siteTitle = sysConfigs.siteName;
    var html = '';
    html += obj.userName + ' （' + obj.email + ') ' + res.__(
        "label_sendRegEmail_text1") + '<br><br>' + '<p>' + siteTitle + ' ' +
      res.__("label_sendRegEmail_text3") + '</p><br>' + '<p>' + res.__(
        "label_sendRegEmail_text2") + ' ' + res.__(
        "label_sendRegEmail_text4") + ' <a href="' + sysConfigs.siteDomain +
      '/users/login" target="_blank">' + res.__("label_sendRegEmail_text5") +
      '</a></p><br><br>';
    return html;
  },
  ...
  renderNoPowerMenus(manageCates, adminPower) {
    ...
    if (!_.isEmpty(adminPower)) {
      ...
      for (let i = 0; i < menuCates.length; i++) {
        ...
        let cateFlag = this.checkNoAllPower(resourceObj._id, optionCates,
          adminPower);
        if (!cateFlag) {
  ...
  renderApiErr(req, res, responseCode, responseMessage, type = '') {
    ...
    if (type == 'save') {
      responseMessage = res.__("resdata_savedata_error", {
        error: responseMessage
      })
    } else if (type == 'delete') {
      responseMessage = res.__("resdata_deldata_error", {
        error: responseMessage
      })
    } else if (type == 'update') {
      responseMessage = res.__("resdata_updatedata_error", {
        error: responseMessage
      })
    } else if (type == 'getlist') {
      responseMessage = res.__("resdata_getlist_error", {
        error: responseMessage
      })
    } else if (type == 'checkform') {
      responseMessage = res.__("resdata_checkformdata_error", {
        error: responseMessage
      })
    }
    let errorData = {
  ...
  getSiteLocalKeys(res) {
    return new Promise((resolve, reject) => {
      cache.get(settings.session_secret + '_localkeys', async(
        localRenderData) => {
        if (!_.isEmpty(localRenderData)) {
          resolve(localRenderData)
        } else {
          const targetLocalJson = require('../locales/zh.json');
          let renderKeys = {};
          // 记录针对组件的国际化信息
          let sysKeys = {};
          ...
          let timeSet = process.env.NODE_ENV === 'production' ? 1000 *
            60 * 60 : 1000;
          cache.set(settings.session_secret + '_localkeys', {
            renderKeys, sysKeys
          }, timeSet);
          resolve({
            renderKeys, sysKeys
  ...
  checkExistFile(tempFilelist, forderArr) {
    ...
    if (filterForderArr.length > 0 && (tempFilelist.length >= forderArr.length) &&
      (filterForderArr.length == tempFilelist.length)) {
      distPath = true;
    }
