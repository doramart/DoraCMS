...
const {
  service, validatorUtil, siteFunc
} = require('../../../utils');
...
function setTempData(res, targetTemp) {
  ...
  //读取ejs模板
  let newPubPath = siteFunc.setTempParentId(service.scanFolder(settings.SYSTEMTEMPFORDER,
    targetTemp + "/public"), 'public');
  let newUserPath = siteFunc.setTempParentId(service.scanFolder(settings.SYSTEMTEMPFORDER,
    targetTemp + "/users"), 'users');
  newPubPath = newPubPath.concat(newUserPath);
  // 读取国际化
  let newI18nPath = siteFunc.setTempParentId(service.scanFolder(process.cwd(),
    '/locales'), 'i18n');
  newPubPath = newPubPath.concat(newI18nPath);
  //读取静态文件
  if (fs.existsSync(settings.TEMPSTATICFOLDER + targetTemp)) {
    let newStylePath = siteFunc.setTempParentId(service.scanFolder(settings.TEMPSTATICFOLDER,
      targetTemp + "/css"), 'styles');
    let newJsPath = siteFunc.setTempParentId(service.scanFolder(settings.TEMPSTATICFOLDER,
      targetTemp + "/js"), 'js');
  ...
  for (let i = 0; i < filePath.length; i++) {
    ...
    if (fileObj.name.split('-')[1] == 'stage') {
      ...
      let unitArr = service.scanFolder(settings.SYSTEMTEMPFORDER, targetTemp +
        '/' + fileObj.name);
...
function getDefaultTempInfo() {
  return new Promise((resolve, reject) => {
    cache.get(settings.session_secret + '_default_temp', async(
      defaultTempData) => {
      if (defaultTempData) {
        ...
        try {
          ...
          if (!_.isEmpty(defaultTemp)) {
            // 缓存1天
            cache.set(settings.session_secret + '_default_temp',
              defaultTemp, 1000 * 60 * 60 * 24);
            resolve(defaultTemp)
          } else {
            resolve([])
          }
...
function checkDistForder(targetPath, forderArr) {
  return new Promise((resolve, reject) => {
    let checkState = siteFunc.checkExistFile(service.scanJustFolder(
      targetPath), forderArr);
    if (checkState) {
      resolve();
    } else {
      let checkTimer = setInterval(() => {
        if (siteFunc.checkExistFile(service.scanJustFolder(targetPath),
            forderArr)) {
...
class ContentTemplate {
  ...
  async getMyTemplateList(req, res, next) {
    try {
      ...
      let renderData = siteFunc.renderApiData(res, 200, 'ContentTemplate',
        temps, 'getlist')
      res.send(renderData);
    } catch (err) {
    ...
    if (filePath && filePath.indexOf('../') >= 0) {
      ...
      if (path) {
        try {
          ...
          let renderData = siteFunc.renderApiData(res, 200,
            'ContentTemplateFile', {
              doc: fileData,
              path: filePath
            }, 'getlist')
  ...
  async updateFileInfo(req, res, next) {
    ...
    form.parse(req, async(err, fields, files) => {
      ...
      if (filePath && filePath.indexOf('../') >= 0 || !fileContent) {
        res.send(siteFunc.renderApiErr(req, res, 500, 'no power',
          'getlist'))
      } else {
        let path = siteFunc.getTempBaseFile(filePath) + filePath;
        if (path) {
          ...
          if (writeState == 200) {
            res.send(siteFunc.renderApiData(res, 200,
              'ContentTemplateFileUpdate', {}, 'update'));
          } else {
            res.send(siteFunc.renderApiErr(req, res, 500, 'no path file',
              'getlist'))
          }
        } else {
          res.send(siteFunc.renderApiErr(req, res, 500, 'no power',
  ...
  async getTempItemForderList(req, res, next) {
    try {
      let defaultTemp = await getDefaultTempInfo();
      // console.log('--defaultTemp----', defaultTemp);
      let filePath = service.scanJustFolder(settings.SYSTEMTEMPFORDER +
        defaultTemp.alias);
      ...
      res.send(siteFunc.renderApiData(res, 200, 'ContentTemplateForder',
        newFilePath, 'getlist'));
    } catch (error) {
      res.send(siteFunc.renderApiErr(req, res, 500, error, 'getlist'))
    }
  ...
  async addTemplateItem(req, res, next) {
    ...
    form.parse(req, async(err, fields, files) => {
      ...
      const newContentTemplateItems = new TemplateItemsModel(
        tempItemObj);
      try {
  ...
  async delTemplateItem(req, res, next) {
    try {
      ...
      res.send(siteFunc.renderApiData(res, 200, 'contentTemplateItems', {},
        'delete'))
  ...
  async getTempsFromShop(req, res, next) {
    ...
    try {
      let templateList = await axios.get(settings.DORACMSAPI +
        '/system/template' + linkParams);
      if (templateList.status == 200) {
        res.send(siteFunc.renderApiData(res, 200, 'contentTemplates',
          templateList.data, 'getlist'))
      } else {
        res.send(siteFunc.renderApiErr(req, res, 500, error, 'getlist'));
      }
    } catch (error) {
  ...
  async installTemp(req, res, next) {
    ...
    try {
      if (tempId) {
        let templateInfo = await axios.get(settings.DORACMSAPI +
          '/system/template/getItem?tempId=' + tempId);
        if (templateInfo.status == 200) {
          // console.log('----templateInfo---', templateInfo)
          let tempObj = templateInfo.data;
          if (_.isEmpty(tempObj)) {
            throw new siteFunc.UserException(res.__(
              "validate_error_params"));
          }
          let file_url = tempObj.filePath;
          let file_targetForlder = tempObj.alias;
          let DOWNLOAD_DIR = settings.SYSTEMTEMPFORDER +
            file_targetForlder.trim() +
            '/';
          let target_path = DOWNLOAD_DIR + url.parse(file_url).pathname.split(
            '/').pop();
            ...
            if (err) {
              ...
              download_file_httpget(file_url, async() => {
                ...
                extract.on('finish', async() => {
                  ...
                  let newTemp = new ContentTemplateModel(
                    tempObj);
                  newTemp.using = false;
                  ...
                  //复制静态文件到公共目录
                  await checkDistForder(settings.SYSTEMTEMPFORDER +
                    tempObj.alias + '/dist', ['images',
                      'css',
                      'js'
                    ]);
                  let fromPath = settings.SYSTEMTEMPFORDER +
                    tempObj.alias + '/dist/';
                  let targetPath = settings.TEMPSTATICFOLDER +
                    tempObj.alias;

                  service.copyForder(fromPath, targetPath);

                  res.send(siteFunc.renderApiData(res, 200,
                    'contentTemplates', {}, 'getlist'))
  ...
  async enableTemp(req, res, next) {
    ...
    try {
      ...
      cache.set(settings.session_secret + '_default_temp', defaultTemp,
        1000 *
        60 * 60 * 24);

      res.send(siteFunc.renderApiData(res, 200, 'enableTemplates', {},
        'update'))
    ...
    try {
      ...
      if (defaultTemp._id == tempId) {
        ...
        if (!_.isEmpty(targetTemp)) {
          ...
          res.send(siteFunc.renderApiData(res, 200, 'uninstallTemp', {},
            'update'))
