...
const templateConfig = {
  ...
  extractfile(ctx, app, DOWNLOAD_DIR, target_path, tempObj, installType) {
    return new Promise((resolve, reject) => {
      ...
      extract.on('finish', async () => {
        ...
        await this._checkDistForder(ctx, app.config.temp_view_forder + tempObj.alias + '/dist', [
          'images',
          'css',
          'js',
        ]);
  ...
  async copyThemeToStaticForder(ctx, app, tempAlias, DOWNLOAD_DIR) {
    ...
    const targetAssetPath = path.join(app.config.baseDir, 'app/assets');
    // 拷贝静态资源
  ...
  async deleteThemeStaticForder(app, tempAlias) {
    ...
    const tempAssetsPath = path.join(app.config.baseDir, `app/assets/${tempAlias}`);
