...
const {
	AdminUser, ContentCategory, Content, ContentTag, User, Message, SystemConfig,
	UserNotify, Ads
...
// 获取用户通知信息
router.get("/getUserNotifys", checkUserSessionForApi, (req, res, next) => {
	req.query.user = req.session.user._id;
	next();
}, UserNotify.getUserNotifys);

// 设置用户消息为已读
router.get("/setNoticeRead", checkUserSessionForApi, (req, res, next) => {
	req.query.user = req.session.user._id;
	next();
...
// 获取用户参与话题
router.get("/getUserReplies", checkUserSessionForApi, (req, res, next) => {
	req.query.user = req.session.user._id;
	next();
}, Message.getMessages);

// 获取用户发布文章
router.get("/getUserContents", checkUserSessionForApi, (req, res, next) => {
	req.query.user = req.session.user._id;
	next();
}, Content.getContents);
