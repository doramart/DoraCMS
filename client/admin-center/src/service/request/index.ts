...
import {BACKEND_ERROR_CODES, BACKEND_ERROR_CODE, createFlatRequest, createRequest } from '@sa/axios';
...
export const request = createFlatRequest<App.Service.Response, RequestInstanceState>(
  ...
  },
  {
    ...
    async onBackendFail(response, instance) {
      ...
      if (responseCode === import.meta.env.VITE_SERVICE_ERROR_CODE) {
        window.$message?.error((response.data as any).message || response.data.msg);
      }
    ...
    onError(error) {
      ...
      if (error.code === BACKEND_ERROR_CODE || BACKEND_ERROR_CODES.includes(error.response?.data?.code||'')) {
...
export const demoRequest = createRequest<App.Service.DemoResponse>(
  ...
  },
  {
    ...
    isBackendSuccess(response) {
      ...
      return String(response.data.status) === '200';
    },
