...
/**
 * 合并同名路由（用于 constant/auth 合并时去重并合并子路由）
 *
 * @param routes 路由列表
 */
export function mergeRoutesByName(routes: ElegantConstRoute[]) {
  const routeMap = new Map<string, ElegantConstRoute>();

  function mergeChildren(
    originChildren: ElegantConstRoute[] | undefined,
    incomingChildren: ElegantConstRoute[] | undefined
  ): ElegantConstRoute[] | undefined {
    if (!originChildren?.length && !incomingChildren?.length) return originChildren || incomingChildren;

    const childMap = new Map<string, ElegantConstRoute>();
    const pushChild = (child: ElegantConstRoute) => {
      const existed = childMap.get(child.name);
      if (!existed) {
        // 深拷贝子节点，避免污染原数据
        const clone = { ...child };
    ...
    (incomingChildren || []).forEach(pushChild);

    return Array.from(childMap.values());
  }

  routes.forEach(route => {
    const existed = routeMap.get(route.name);
    if (!existed) {
      const clone = { ...route };
      if (clone.children) clone.children = mergeChildren([], clone.children);
