...
async function buildTemplates() {
  ...
  try {
    ...
    // 2. 复制 server
    const serverStats = await copyWithFilter(
    ...
    // 3. 复制 client
    const clientStats = await copyWithFilter(
    ...
    if (await fs.pathExists(scriptsSource)) {
      const scriptsStats = await copyWithFilter(
        scriptsSource,
    ...
    if (await fs.pathExists(dockerSource)) {
      const dockerStats = await copyWithFilter(
        dockerSource,
    ...
    await fs.writeJSON(
      path.join(templatesDir, '.template-metadata.json'),
      metadata,
      { spaces: 2 }
    );
